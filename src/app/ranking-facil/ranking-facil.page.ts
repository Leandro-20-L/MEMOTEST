...
@Component({
  selector: 'app-ranking-facil',
  templateUrl: './ranking-facil.page.html',
  styleUrls: ['./ranking-facil.page.scss'],
  standalone: true,
  imports: [IonContent, IonHeader, IonTitle, IonToolbar, CommonModule, FormsModule,IonList,IonLabel,IonItem, IonItemDivider]
})
export class RankingFacilPage implements OnInit {
  ...
  async ngOnInit() {
      ...
      if (error) {
        ...
        this.topFacil = filteredResults
          .filter(r => r.dificultad === 'Fácil')
          .slice(0, 5);
