...
@Component({
  ...
  standalone: true,
  imports: [IonContent, IonHeader, IonTitle, IonToolbar, CommonModule, FormsModule,IonButton,IonCard]
})
export class DificilPage implements OnInit {
  ...
  cards: any[] = [];
    ...
    async ngOnInit() {
      this.generateCards();
      const id = await this.authService.getUserUid() || "";
      this.usuario = await this.dataService.getUserNameByUID(id);
    }
    ...
    checkGameFinished() {
      ...
      if (allFlipped) {
        ...
        this.dataService.saveGameResult(this.usuario, this.elapsedTime, "Difícil");
      }
    }
